...
void MyOpenGLWidget::paintGL()
{
    ...
    if(!vertex_non){
        ...
        if(vertex_circle)  glEnable(GL_POINT_SMOOTH);
        else if (vertex_square) glDisable(GL_POINT_SMOOTH);


        glBegin(GL_POINTS);
        for (int i = 0; i < data3d.v->amount_vert; i++) {
            glColor3f(vertex_color.redF(), vertex_color.greenF(), vertex_color.blueF());
            glVertex3d(data3d.v->coord[i].x, data3d.v->coord[i].y, data3d.v->coord[i].z);
        }
        glEnd();
    }
    ...
    for (int i = 0; i < data3d.amount_polygon; i++) {
        ...
        for (int j = 0; j < data3d.p[i].amount_p; j++) {
            int vertexIndex = data3d.p[i].p[j] - 1;
            glColor3f(line_color.redF(), line_color.greenF(), line_color.blueF());
            glVertex3f(data3d.v->coord[vertexIndex].x, data3d.v->coord[vertexIndex].y, data3d.v->coord[vertexIndex].z);
        }
        glEnd();
    }
    if(!solid_edge){
