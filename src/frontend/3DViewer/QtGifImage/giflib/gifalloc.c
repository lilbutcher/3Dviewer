    ...
    for (i = 0; i < Len; i += 4) {
      for (j = 0; j < 4 && j < Len; j++) {
        (void)fprintf(fp, "%3d: %02x %02x %02x   ", i + j,
                      Object->Colors[i + j].Red, Object->Colors[i + j].Green,
...
ColorMapObject *GifUnionColorMap(const ColorMapObject *ColorIn1,
  ...
  if (ColorUnion == NULL) return (NULL);
...
SavedImage *GifMakeSavedImage(GifFileType *GifFile,
  ...
  if (GifFile->SavedImages == NULL)
    ...
    GifFile->SavedImages = (SavedImage *)realloc(
        GifFile->SavedImages, sizeof(SavedImage) * (GifFile->ImageCount + 1));
